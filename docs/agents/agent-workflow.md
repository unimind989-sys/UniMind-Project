...
3. Run guarded local `pnpm verify` when the task or policy selects broad stable verification; CI uses `pnpm verify:ci` at the exact candidate head.
   The local gate compares the candidate fingerprint again after the broad chain. A changed candidate invalidates that run, including an otherwise green result. Freeze tracked inputs until the command exits.
   For a database change that needs earlier disposable execution, push the work branch and manually dispatch CI with `database_feedback=true`; this runs only the guarded database job. It is feedback, so the final PR head still needs every required check.
4. While conditional CI is `SHADOW` or `READY`, keep broad CI unchanged, retain the router's per-job run/would-skip predictions, and compare them with outcomes from that same broad run. READY requires the policy's complete regression coverage, run and would-skip observations for every job, and zero unsafe skip contradictions; a contradiction falls back only the affected job. ENFORCED requires a separate governed policy and CI workflow change.
   In `ENFORCED`, the required `ci-selector` check validates the policy, workflow fingerprint, readiness evidence, and exact PR comparison commits before selecting jobs. Safe Markdown-only PRs run formatting, secret scanning, and readiness checks while the three heavyweight jobs report skipped success. Policy/workflow changes, test-input documents, unknown paths, and selector failures run all jobs; a failed selector also blocks merge. Main pushes retain full application/database proof. Required check names remain present.
