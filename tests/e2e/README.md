...
`pnpm test:e2e` starts the app on `127.0.0.1:3100` with safe synthetic environment values, mock mode, zero budget, one Chromium worker, a 120-second bounded server-start allowance, a 15-second test timeout, and external browser requests blocked by the test. Playwright Test is the repeatable gate; Codex uses its in-app side browser for ordinary interactive work. The project-pinned `pnpm browser:cli` command is reserved for explicit `$playwright-cli` tracing or test-debugging tasks.
