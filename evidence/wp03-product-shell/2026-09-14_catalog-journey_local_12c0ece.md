...
| Safe access state | Empty/error state reveals no private labels, IDs, or diagnostics | A second security-invoker RPC returns one of six fixed safe codes through a private bounded evaluator; the adapter rejects unknown codes, unknown progression modes, and row/state disagreement | PASS pending disposable pgTAP | Adapter unit tests 4/4; security matrix 1,590 decisions |
