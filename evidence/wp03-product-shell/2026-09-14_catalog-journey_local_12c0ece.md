...
**Status:** PASS — superseded by protected release evidence
...
**Migrations:** candidate migration `20260914112000_student_catalog_journey.sql`; subsequently applied and verified in Supabase Preview under the protected release report
...
| --- | --- | --- | --- | --- |
| Caller-scoped catalog | Every visible row originates from current caller authorization | `available_catalog_entries()` begins with `available_curriculum_units(false)`, uses security-invoker/RLS semantics, and returns the configured path only for AVAILABLE units | PASS | Migration; `supabase/tests/25_student_catalog_journey.sql`; GitHub run `34957861352` |
| Safe access state | Empty/error state reveals no private labels, IDs, or diagnostics | A second security-invoker RPC returns one of six fixed safe codes through a private bounded evaluator; the adapter rejects unknown codes, unknown progression modes, and row/state disagreement | PASS | Adapter unit tests 4/4; security matrix 1,590 decisions; GitHub run `34957861352` |
...
| 2026-09-14 | `corepack pnpm test:security` | 0 | 22/22 tests passed after the original catalog candidate extended the actor/action/resource matrix from 38 to 40 functions and 1,572 to 1,584 reviewed decisions. The later private safe-state evaluator brings the current totals to 41 functions and 1,590 decisions. |
...
| 2026-09-15 | GitHub Actions run `34955820545` | 1 | Dependency/application gates, Vercel Preview, upgrade, two clean resets, migration parity, the public-function matrix, and all 19 journey assertions executed. Two semantic assertions showed that caller RLS intentionally hides unpublished/no-source units, causing the invoker state seam to collapse those conditions to `NO_CATALOG`; the disposable environment was cleaned and no shared environment changed. |
| 2026-09-15 | Safe-state seam correction: `corepack pnpm check:sql`; `corepack pnpm test:security`; diff and secret scans | 0 | Kept the public non-identifying state RPC as a security invoker and bound it at definition time to one private bounded security-definer evaluator. Identity remains `auth.uid()`-derived, output is a fixed safe-code allowlist, both search paths are empty, authenticated callers retain no private-schema usage, and catalog identifiers continue through the security-invoker/RLS function. |
| 2026-09-15 | GitHub Actions run `34957051902` | 1 | Upgrade, two clean resets, migration parity, all 370 pgTAP assertions including the 19 journey checks, database advisors, and generated-type production passed. The final parity check rejected only the alphabetical position of `progression_mode` in the checked-in generated TypeScript; the disposable environment was cleaned and no shared environment changed. |
| 2026-09-15 | Generated-type ordering repair | 0 | Moved the existing `progression_mode` declarations after `program_type` in Row/Insert/Update to match the hosted Supabase generator exactly; no type or runtime behavior changed. |
...
| Expired membership | No catalog identifiers; generic safe state only | Database contract returns no rows and `NO_MEMBERSHIP`; preview payload contains no option controls | PASS |
| Cohort locks while browsing | Next server operation removes visible catalog | Safe-state preview replaces all identifiers with `COHORT_LOCKED`; pgTAP mutates and rolls back the synthetic release | PASS |
| Published unit loses READY source | Unit disappears and safe cause remains bounded | RPC state allowlist returns `READY_SOURCE_MISSING`; browser sees only localized generic copy | PASS |
| No catalog / all units unpublished | Distinct safe state without identifiers | Database test constructs an empty cohort and withdraws the published synthetic unit inside savepoints | PASS |
...
| --- | --- | --- | --- | --- | --- |
| WP03-T03-D01 | Closed environment constraint | The guarded disposable Supabase/pgTAP runner refuses Windows and required GitHub-hosted Linux. Final run `34957861352` executed all 370 assertions including the 19 journey checks. | Delivery agent / GitHub CI | Closed 2026-09-15 | None |
...
| WP03-T03-D06 | Closed semantic defect | The third disposable run proved that caller RLS correctly hides unpublished and no-READY-source units, so an invoker state RPC could not distinguish those safe conditions from an empty catalog. The public RPC remains an invoker and delegates through a stored, definition-time-bound call to one private fixed-code evaluator with `auth.uid()` identity, empty search paths, explicit EXECUTE, and no authenticated private-schema usage; the identifier-bearing catalog RPC remains RLS-scoped. | Delivery agent | Closed 2026-09-15 | Replacement database CI |
| WP03-T03-D07 | Closed generated parity defect | The fourth disposable run passed every database behavior and advisor check, then found that the manually synchronized `progression_mode` property preceded `program_type` while the hosted generator sorts it after. The three declarations were reordered with no semantic change. | Delivery agent | Closed 2026-09-15 | Replacement database CI |
...
- [x] The candidate phase made no paid call or shared mutation. The later protected release remained mock-only, zero-budget, and is recorded separately.

## Rollback/disable procedure

This candidate report is superseded by the protected release report, whose rollback procedure governs the deployed state. Never rewrite applied migration history.

## Decision

The revised local technical and UX candidate passed, then completed protected delivery. Exact-head GitHub run `34957861352`, merged-main run `34979935225`, Supabase Preview hosted postflight, and Vercel Production verification are recorded in `2026-09-15_catalog-journey-release_production_1ee9593.md`.

| Name | Role | Decision | Date |
| --- | --- | --- | --- |
| Codex `/root` | Agent executor | TECHNICAL PASS; protected release evidence recorded separately | 2026-09-15 |
