---
name: taste
description: Give a marketing page, landing page, portfolio, or marketing-site redesign a stronger visual direction. Use only when the user explicitly invokes $taste; UniMind product screens use impeccable.
---
...
This is the pinned upstream Taste Skill with a project scope boundary:

- Use it only after the user explicitly invokes `$taste` for a visitor-facing marketing surface.
- Use `impeccable` for UniMind application screens, dashboards, and multi-step product flows.
- Read root `DESIGN.md` when it exists and preserve confirmed project decisions. Repository authorities and accessibility requirements override aesthetic preferences below.
- Treat motion libraries and new UI dependencies as implementation choices that require an existing project fit; do not add one merely because an upstream example names it.
- Finish with rendered browser verification and the pinned Vercel guidelines. Do not claim fidelity from code inspection alone.
