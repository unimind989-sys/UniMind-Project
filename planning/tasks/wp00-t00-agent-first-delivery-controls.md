...
**Reviewer:** Ahmed approved finalization of the lean-execution, tool-access, and browser-routing correction through the explicit `$finalize` invocation on 2026-09-15
...
**Evidence:** `evidence/wp00-pilot/2026-09-14_autonomous-finalize_local_a3178fb.md` records the autonomous-finalization adaptation; `evidence/wp00-pilot/2026-09-16_agent-readiness_local_5fd79cc.md` records the completed WP00-T00 checkpoint and lean-tooling verification.
...
- [x] Audit entry points, task selection, naming, verification, and handoff coverage.
- [x] Add the agent-first operating workflow and task-selection rule.
- [x] Add the task-record template and planning location.
- [x] Add the readiness verifier and make its focused checks pass.
- [x] Add and machine-check the read-only work-state recommendation.
- [x] Rehearse the repository-only handoff in an isolated committed snapshot.
- [x] Pin and scope the requested visual reference, image-to-code, browser, and review toolchain.
- [x] Add the manual `$finalize` workflow with adaptive impact tiers, rate-limit discipline, protected release gates, affected-service verification, and cleanup.
- [x] Adapt `$finalize` so explicit invocation autonomously carries both founders' standing authorization through every selected-task non-financial gate, merge, affected-service promotion, evidence, and verified local/remote branch cleanup; preserve a fresh exact-cost confirmation only for real-money exposure.
- [x] Make verification impact-based and non-overlapping so passing checks are broadened or repeated only after a relevant invalidation.
- [x] Route internal browser work to the in-app side browser, external review to Chrome, and Playwright CLI to explicit specialist use.
- [x] Audit current GitHub, Supabase, Vercel, rendered-browser, and CI access and record the smallest justified operational set without installing or authorizing anything.
- [x] Obtain an Ahmed-or-Ziad human checkpoint and create commit-specific evidence.
