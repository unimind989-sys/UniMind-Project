...
- **Mutation rule:** Business mutations call `requireVerifiedIdentity()` and use the request-scoped server client so database RLS remains the final authorization boundary. Auth-cookie mutations must also provide the server client with a response-header propagation sink.
